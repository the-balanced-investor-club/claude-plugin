...
8. **Catalog version: 0.2.0.** Connector responses from `start_here`/`about_us` report the latest published catalog — if it's newer than this one, tell the user their plugin has an update (restarting Claude auto-updates it).
...
- [ ] Section headers formatted per user's template (default: dark blue #28333C with white bold text)
